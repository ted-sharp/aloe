...
**Canvas 2D API** でカレンダーを描画する（Konva.js から移行）。
...
**日付選択操作**
- シングルクリック: 期間の開始日を設定（検索フィルターの開始日に反映）
- Shift+クリック: 期間の終了日を設定（検索フィルターの終了日に反映）
- ドラッグ: 範囲選択で期間を設定
- ダブルクリック: その日の詳細表示に切り替え

**検索フィルター**
- 期間: 開始日・終了日で絞り込み（カレンダー選択と連動）
- 曜日: 複数選択可能（デフォルトは全オフ）
- 時間帯: 複数選択可能（バッジスタイルUI）
- 診療室: 複数選択可能
- 設備: 複数選択可能
- フィルター非該当の日付・時間帯はグレーアウト表示

**凡例表示**
- 充足率と色の対応関係
- 状態表示（フィルター非該当、選択中、今日）
- 操作方法の説明
- グラフの見方
