...
public class VoiceCommandParserTests
{
    ...
    public VoiceCommandParserTests()
    {
        ...
        this._parser = new VoiceCommandParser(mockDateTime.Object);
    }
    ...
    {
        this._parser.Parse(transcript).Intent.Should().Be(expected);
    }
    ...
    {
        this._parser.Parse("こんにちは").Intent.Should().Be(VoiceCommandIntent.Unknown);
    }

    [Fact]
    public void Parse_空文字_ReturnsUnknown()
    {
        this._parser.Parse("").Intent.Should().Be(VoiceCommandIntent.Unknown);
    }
    ...
    {
        var result = this._parser.Parse("明日の10時に予約を入れて");
    ...
    {
        var result = this._parser.Parse("3月5日の10時に予約");
    ...
    {
        var result = this._parser.Parse("午後3時半に予約を入れて");
    ...
    {
        var result = this._parser.Parse("明日の10時に田中さんの予約を入れて");
    ...
    {
        var result = this._parser.Parse("山田様の予約をキャンセル");
    ...
    {
        var result = this._parser.Parse("鈴木の予約を確認して");
    ...
    {
        var result = this._parser.Parse("3月5日の10時に田中さんの予約を入れて");
    ...
    {
        var result = this._parser.Parse("来週の月曜日の予約を確認して");
    ...
    {
        var result = this._parser.Parse("明日の十時に予約を入れて");
        result.StartMin.Should().Be(600);
    }
    ...
    {
        var result = this._parser.Parse("三月五日の十時半に田中さんの予約を入れて");
        result.Date.Should().Be(new DateOnly(2026, 3, 5));
    ...
    {
        var result = this._parser.Parse("明日の、10時に、田中さんの予約を入れて。");
        result.Intent.Should().Be(VoiceCommandIntent.CreateAppointment);
        result.Date.Should().Be(Today.AddDays(1));
        result.StartMin.Should().Be(600);
        result.PatientName.Should().Be("田中");
    ...
    {
        var result = this._parser.Parse("午後三時に予約を入れて");
    ...
    {
        var result = this._parser.Parse("おとついの予約を確認して");
        result.Intent.Should().Be(VoiceCommandIntent.SearchAppointment);
    ...
    {
        var result = this._parser.Parse("おとといの予約を確認して");
        result.Date.Should().Be(Today.AddDays(-2));
    }
    ...
    {
        var result = this._parser.Parse("きのうの予約を確認して");
        result.Date.Should().Be(Today.AddDays(-1));
    }
    ...
    {
        var result = this._parser.Parse("しあさってに予約を入れて");
    ...
    {
        var result = this._parser.Parse("来月のついたちに予約を入れて");
        result.Intent.Should().Be(VoiceCommandIntent.CreateAppointment);
    ...
    {
        var result = this._parser.Parse("来月のはつかに予約を入れて");
    ...
    {
        var result = this._parser.Parse("明日の午後いちに予約を入れて");
    ...
    {
        var result = this._parser.Parse("明日の朝いちに予約を入れて");
    ...
    {
        var result = this._parser.Parse("明日のお昼に予約を入れて");
    ...
    {
        var result = this._parser.Parse("きょうの予約を確認して");
        result.Date.Should().Be(Today);

        var result2 = this._parser.Parse("あしたの10時に予約を入れて");
    ...
    {
        var result = this._parser.Parse(transcript);
    ...
    public void Parse_午後に_は助詞なので時刻なし()
    {
        ...
        var result = this._parser.Parse("午後に予約を入れて");
    ...
    {
        var result = this._parser.Parse("午後さん予約を入れて");
    ...
    public void Parse_午前いち_Returns午前1時扱い()
    {
        ...
        var result = this._parser.Parse("午前いちに予約を入れて");
    ...
    {
        var result = this._parser.Parse("午後よんに予約を入れて");
    ...
    {
        var result = this._parser.Parse("午後ご予約を入れて");
    ...
    {
        // 「し」→ 4
        var result = this._parser.Parse("午後しに予約を入れて");
        result.StartMin.Should().Be(960); // 16:00
    }
    ...
    {
        this._parser.Parse(transcript).Intent.Should().Be(VoiceCommandIntent.TentativeAppointment);
    }
    ...
    {
        var result = this._parser.Parse("月曜だけ表示して");
    ...
    {
        var result = this._parser.Parse("月曜と火曜と水曜だけ表示して");
        result.Intent.Should().Be(VoiceCommandIntent.SetFilter);
    ...
    {
        var result = this._parser.Parse("午前のみ表示して");
        result.Intent.Should().Be(VoiceCommandIntent.SetFilter);
    ...
    {
        var result = this._parser.Parse("午後だけ表示して");
        result.Intent.Should().Be(VoiceCommandIntent.SetFilter);
    ...
    {
        var result = this._parser.Parse("フィルタークリア");
        result.Intent.Should().Be(VoiceCommandIntent.SetFilter);
    ...
    {
        var result = this._parser.Parse("フィルターリセットして");
        result.Intent.Should().Be(VoiceCommandIntent.SetFilter);
        result.FilterParams!.ClearAll.Should().BeTrue();
    }
    ...
    {
        var result = this._parser.Parse("全部表示して");
        result.Intent.Should().Be(VoiceCommandIntent.SetFilter);
        result.FilterParams!.ClearAll.Should().BeTrue();
    }
    ...
    {
        var result = this._parser.Parse("空きが2件以上の日を表示して");
        result.Intent.Should().Be(VoiceCommandIntent.SetFilter);
    ...
    {
        var result = this._parser.Parse("絞り込みして");
        result.Intent.Should().Be(VoiceCommandIntent.SetFilter);
    }
}
