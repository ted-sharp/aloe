// <copyright file="SkiaSheetRenderer.cs" company="ted-sharp">
...
/// <summary>
/// SkiaSharp を使用して <see cref="SheetModel"/> を <see cref="SKCanvas"/> に描画する共有レンダラー。
/// QuestPdfWriter と WindowsSheetPrinter の両方から使用される。
/// </summary>
public class SkiaSheetRenderer
{
    private readonly PositionCalculator _calc;
    private readonly SystemFontResolver _fontResolver = new();
    private readonly ILogger<SkiaSheetRenderer> _logger;

    /// <summary>
    /// <see cref="SkiaSheetRenderer"/> クラスの新しいインスタンスを初期化する。
    /// </summary>
    /// <param name="calc">セル座標計算機。</param>
    /// <param name="logger">ロガー。省略時は NullLogger を使用する。</param>
    public SkiaSheetRenderer(PositionCalculator calc, ILogger<SkiaSheetRenderer>? logger = null)
    {
        _calc = calc;
        _logger = logger ?? NullLogger<SkiaSheetRenderer>.Instance;
    }

    /// <summary>
    /// <see cref="SheetModel"/> を指定の <see cref="SKCanvas"/> に描画する。
    /// </summary>
    /// <param name="canvas">描画先キャンバス。座標単位はポイント(pt)。</param>
    /// <param name="model">描画するシートモデル。</param>
    public void Render(SKCanvas canvas, SheetModel model)
    {
